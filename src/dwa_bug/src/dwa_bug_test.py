...
class Config():
	...
	def __init__(self):
		...
		self.odom = rospy.wait_for_message("base_pose_ground_truth", Odometry, timeout=None)
		...
		self.current_v = self.odom.twist.twist.linear.x
		self.current_w = self.odom.twist.twist.angular.z
...
def calc_final_input(x, u, dw, config, obst):
	...
	for v in np.arange(dw[0], dw[1]+0.5*config.v_reso,config.v_reso):
		for w in np.arange(dw[2], dw[3]+0.5*config.yawrate_reso, config.yawrate_reso):
			...
			final_cost = 0.1 * odom_angle_cost + 8.0*ob_cost +2.0*to_goal_cost #+ 2.5*speed_cost
...
def main():
	...
	#subOdom = rospy.Subscriber("base_pose_ground_truth", Odometry, config.assignOdomCoords,queue_size=1)
	subLaser = rospy.Subscriber("/base_scan", LaserScan, obs.assignObs2, config,queue_size=1)
	...
	#while config.current_v >=0:
		...
		if (atGoal(config,x) == False):#usual use
			...
			#print(x)
			u,state = dwa_control(x, [x[3],x[4]], config, obs.obst)
			#x[0] = config.x
			#x[1] = config.y
			#x[2] = config.th
			#x[3] = u[0]
