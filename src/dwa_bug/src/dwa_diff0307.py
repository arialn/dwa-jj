...
class Config():
	...
	def __init__(self):
		...
		self.save_result_figure = False
		self.save_process_time = False

		self.result_path = '../results/{}/'.format(time.strftime("%Y%m%d", time.localtime()))

		self.type = "jj"  # dwa a a2 j
		# self.laser_reso = []
		...
		if isinstance(config.path, list) == True:
			if len(config.path) == 0:
				...
				for i in range(len(config.path) - 1, -1, -1):
					path_in_robot = np.dot(ANTI_ABT, [config.path[i][0], config.path[i][1], 0, 1])
					number = int(np.rad2deg(math.atan2(path_in_robot[1], path_in_robot[0])))
					if (math.sqrt((config.path[i][0] - config.x) ** 2 + (
							config.path[i][1] - config.y) ** 2)) < config.sensor_range and \
							(math.sqrt((config.path[i][0] - config.x) ** 2 + (config.path[i][1] - config.y) ** 2)) < \
							msg.ranges[number]:
...
def calcFinalInput(x, u, dw, config, obst, obst_rc):
	...
	if config.type == "dwa":
		...
		for av in np.arange(-config.max_linear_acceleration,
			...
			for w in np.arange(dw[2], dw[3] + 0.5 * config.angular_velocity_resolution,
				...
				speed_cost = np.nanmean(v_predict_list) / config.max_linear_velocity
				final_cost = (1.0 * to_path_cost + 1.0 * to_goal_cost1 + 1.0 * ob_cost1 + config.v_weight * speed_cost)
		...
		for av in np.arange(-config.max_linear_acceleration,
			...
			for w in np.arange(dw[2], dw[3] + 0.5 * config.angular_velocity_resolution,
				...
				speed_cost = np.nanmean(v_predict_list) / config.max_linear_velocity
				final_cost = (
						0.0 * odom_angle_cost + 1.0 * to_goal_cost2 + 2.0 * ob_cost2 + config.v_weight * speed_cost)
		...
		for jv in np.arange(-config.max_linear_jerk, config.max_linear_jerk + 0.5 * config.linear_jerk_resolution,
							config.linear_jerk_resolution):
			for jw in np.arange(-config.max_angular_jerk,
								config.max_angular_jerk + 0.5 * config.angular_jerk_resolution,
								config.angular_jerk_resolution):
				val = []
				vssl = []
				wal = []
				wssl = []
				...
				to_path_cost = 0  # calcToPathCost(config, traj)
				speed_cost = np.nanmean(v_predict_list) / config.max_linear_velocity
				final_cost = 1.0 * to_goal_cost1 + 2.0 * ob_cost1 + \
...
def result_graph(config, mode, element1, element2=None, element3=None):
	...
	ax_element3.axis['right2'] = element3_axisline(loc='right', axes=ax_element3, offset=(80, 0))
	...
	if mode == 'waj':
		...
		ax_element1.set_xlabel('Time [s]')
		ax_element2.set_ylabel('Acceleration [' + 'rad/$ \mathit{s}^{2}$' + ']')
		ax_element3.set_ylabel('Jerk [' + 'rad/$ \mathit{s}^{3}$' + ']')
	...
	if mode == 'right':
		...
		ax_element2.set_ylabel('Acceleration [' + 'm/$ \mathit{s}^{2}$' + ']')
		ax_element3.set_ylabel('Jerk [' + 'm/$ \mathit{s}^{3}$' + ']')
		ax_element1.set_ylim(0, config.max_linear_velocity + 0.5)
...
def main():
	...
	f = open(config.result_path + '0509-1' + config.type + '.txt', "a")
