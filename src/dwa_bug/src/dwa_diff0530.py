...
class Obstacles():
	...
	def assignObs(self, msg, config):
		...
		transform_matrix = np.array([[math.cos(config.th), -math.sin(config.th), 0, config.x],
									 ...
									 [0, 0, 1, 0],
									 [0, 0, 0, 1]])
		goal_in_world_coordinate = np.array([config.goalX, config.goalY, 0, 1])
		transform_matrix_anti = np.linalg.inv(transform_matrix)
		#goal_in_robot_coordinate = np.dot(transform_matrix_anti, goal_in_world_coordinate)
		for angle in self.myRange(0, deg - 1, 1):
			# for angle in self.myRange(int(deg*0.25),int(deg*0.75),1):
			distance_from_obstacle = msg.ranges[angle]
			if distance_from_obstacle < config.sensor_range:
				angle_in_sensor = -math.pi + angle * msg.angle_increment
				# x_in_robot_coordinate = distance_from_obstacle * math.cos(angle_in_sensor)  # obstacle in robot coordinate
				# y_in_robot_coordinate = distance_from_obstacle * math.sin(angle_in_sensor)  # obstacle in robot coordinate
				obstacle_in_robot_coordinate = np.array([distance_from_obstacle * math.cos(angle_in_sensor),
														 ...
														 0, 1])
				obstacle_in_world_coordinate = np.dot(transform_matrix,
													  obstacle_in_robot_coordinate)  # obstacle in world coordinate
				self.obst.add((obstacle_in_world_coordinate[0], obstacle_in_world_coordinate[1]))
				self.obst_rc.add((distance_from_obstacle * math.cos(angle_in_sensor),
								  distance_from_obstacle * math.sin(angle_in_sensor)))
		...
		if isinstance(config.path, list):
			if len(config.path) == 0:
				...
				for i in range(len(config.path) - 1, -1, -1):
					path_in_robot = np.dot(transform_matrix_anti, [config.path[i][0], config.path[i][1], 0, 1])
	...
	if config.type == "dwa":
		...
		for av in np.linspace(-config.max_linear_acceleration, config.max_linear_acceleration, 5):
			for w in np.linspace(dw[2], dw[3], 5):
				...
				final_cost = (5.0 * to_goal_cost2 + 1.0 * ob_cost2)
...
def calcObstacleCost2(config, obst_rc, av, v_predict_list, w):
	...
	if w0 != 0:
		...
		if len(obst_rc_list) == 0:
			...
			obst_rc_array = np.array(obst_rc_list)
			inrange_condition = np.logical_and((0 < obst_rc_array[:, 0]),
												(obst_rc_array[:, 0] < xend))
			obst_inrange = obst_rc_array[inrange_condition]
			obst_outrange = obst_rc_array[~inrange_condition]
			if np.any(obst_inrange):
