...
class Obstacles():
	...
	def assignObs2(self, msg, config):
		deg = len(msg.ranges)   # Number of degrees - varies in Sim vs real world
		reject1=np.array([0,0])
		reject2=np.array([0,0])
		self.obst = set()   # reset the obstacle set to only keep visible objects
		self.sudden = list()
		...
		for angle in self.myRange(0,deg-1,1):
			...
			if (distance < 5.0):
				...
				xx = xb*dd/distance
				yy = yb*dd/distance
				BBP = np.array([xx,yy,0,1])
				AAP = np.dot(ABT, BBP)

				reject1 = reject1+np.array([config.x-AAP[0],config.y-AAP[1]])
				reject2 = reject1+np.array([config.x-AP[0],config.y-AP[1]])

		#print(AP)
		#print(AAP)
		#print(AP[0],AP[1],math.sqrt((AP[0]-config.x)**2+(AP[1]-config.y)**2))
		#print(AAP[0],AAP[1],math.sqrt((AAP[0]-config.x)**2+(AAP[1]-config.y)**2))
...
def calc_final_input(x, u, dw, config, obst):
	...
	for v in np.arange(dw[0], dw[1]+config.v_reso,config.v_reso):
		for w in np.arange(dw[2], dw[3], config.yawrate_reso):
			...
			traj_angle_cost = calc_traj_angle_cost(traj,config)
