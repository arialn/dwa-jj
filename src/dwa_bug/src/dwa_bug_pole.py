...
def calc_trajectory0(xinit, v, w, config):
	x = np.array(xinit)
	#print(x)
	...
	if w < 0:
		...
		x_pole = -20
		r_path = abs(v/w)
		theta1 = 0.5*math.pi-(abs(w)*config.predict_time)
		theta2 = 0.5*math.pi
		x_end = r_path * math.cos(theta1)+0
		y_end = r_path * math.sin(theta1)+y_pole
		#print(v,w,math.sqrt((traj[-2,0]-x_pole)**2+(traj[-2,1]-y_pole)**2),y_pole,r_path)
	elif w > 0:
		y_pole = (v/w)
		x_pole = -20
		r_path = abs(v/w)
		theta1 = -0.5*math.pi
		theta2 = -0.5*math.pi+(abs(w)*config.predict_time)
		x_end = r_path * math.cos(theta2)+0
		y_end = r_path * math.sin(theta2)+y_pole
		#print(v,w,math.sqrt((traj[-2,0]-x_pole)**2+(traj[-2,1]-y_pole)**2),y_pole ,r_path)
	else:
		x_end = v * config.predict_time
		y_end = 0
	ABT = np.array([[math.cos(config.th),-math.sin(config.th),0,config.x],
					[math.sin(config.th),math.cos(config.th),0,config.y],
					[0,0,1,0],
	...
	ANTI_ABT = np.linalg.inv(ABT)
	#print(ABT)
	goal = np.dot(ANTI_ABT,[traj[-1][0],traj[-1][1],0,1])
	#print(v,w)
	#fig = plt.figure()
	...
	#plt.show()
	#print(v,w,x_end,y_end,goal[0],goal[1])
	#print(math.sqrt((traj[-1,0]-x_pole)**2+(traj[-1,1]-y_pole)**2),r_path)
	return traj
def calc_trajectory(v, w, config):
	if w > 0:
		y_pole = v/w
		r_path = abs(v/w)
		theta1 = -0.5*math.pi
		...
		theta1 = None
		theta2 = None
	return y_pole,r_path,theta1,theta2
...
def calc_final_input(x, u, dw, config, obst):
	...
	#for v in np.arange(1, 4,0.5):
	#	for w in np.arange(-1, 1.1, 0.5):
	for v in np.arange(dw[0], dw[1]+0.5*config.v_reso,config.v_reso):
	...
	#for v in np.linspace(dw[0], dw[1],num=5):
		#for w in np.linspace(dw[2], dw[3],num=5):
			v = 2
			#traj = calc_trajectory0(xinit, v, w, config)
			y_pole,r_path,theta1,theta2 = calc_trajectory(v, w, config)# generate trajectory
			#plt.plot(traj[:,0],traj[:,1],color='black',linewidth=1)
			#plt.pause(0.01)

			to_goal_cost = calc_to_goal_cost(v,w,y_pole,r_path,theta1,theta2,config)# score from end of trajectory to goal
			...
			speed_cost = v/config.max_speed

			ob_cost = calc_obstacle_cost(v,w,y_pole,r_path,theta1,theta2, obst, config)#score from end of trajectory to obstacle
			#print(v,w,ob_cost)
			#currrent_ob_cost = current_calc_obstacle_cost(obst, config)#score from current position to obstacle

			odom_angle_cost = calc_odom_angle_cost(v,w,y_pole,r_path,theta1,theta2,config)#准确度？
			#odom_angle_cost1 = calc_odom_angle_cost0(traj,config)
			#print(v,w,ob_cost)
			#traj_angle_cost = calc_traj_angle_cost(traj,config)#弯曲度？
			#if odom_angle_cost1 == odom_angle_cost2:
			#	print('True')
				#pass
			#else:
			#	print('False')
			...
			#print(v,w,odom_angle_cost1,odom_angle_cost1)
			...
			if config.state == "traped":
				final_cost2 = (0.0*odom_angle_cost1+1.0*to_goal_cost+2.0*ob_cost +2.0*speed_cost)
				if final_cost2 >= max_cost:
					max_cost = final_cost2
					#best_to_goal = to_goal_cost
					max_u = [v, w]
...
def calc_obstacle_cost(v,w,y_pole,r_path,theta1,theta2, obst, config):
	min_r =10000
	#v = 2
	#print(w,r_path)
	#w = 0
	if w < 0:
		if r_path > 1.2:
			for i in obst.copy():
				theta_obst = math.atan2((i[1]-y_pole),(i[0]))
				r_obst = math.sqrt((i[0])**2+(i[1]-y_pole)**2)
				if theta1 <= theta_obst <= theta2:
					r = abs(r_obst-r_path)
				elif theta1-math.asin(1.2/r_path) < theta_obst < theta1:
					r = math.sqrt(abs((r_path**2)+(r_obst**2)-(2*r_path*r_obst*math.cos(theta1-theta_obst))))
				else:
					r = 10000
				if r < min_r:
					min_r = r
			...
			for i in obst.copy():
				theta_obst = math.atan2((i[1]-y_pole),(i[0]))
				r_obst = math.sqrt((i[0])**2+(i[1]-y_pole)**2)
				dist_end = math.sqrt(r_path**2+r_obst**2-2*r_path*r_obst*math.cos(theta1-theta_obst))
				if r_obst > r_path:
					r = r_obst-r_path
				else:
					r = max(abs(r_obst-r_path),dist_end)
				if r < min_r:
					min_r = r
		#print(v* math.cos(w)*config.predict_time,v* math.sin(w) * config.predict_time)
		#print(v,w,min_r)
	elif w > 0:
		if r_path > 1.2:
			for i in obst.copy():
				...
				if theta1 <= theta_obst <= theta2:
					r = abs(r_obst-r_path)
				elif theta2 < theta_obst < theta2+math.asin(1.2/r_path):
					#print(theta2,theta_obst)
					r = math.sqrt(abs((r_path**2)+(r_obst**2)-(2*r_path*r_obst*math.cos(theta2-theta_obst))))
				else:
					r = 10000
				if r < min_r:
					min_r = r
			...
			for i in obst.copy():
				theta_obst = math.atan2((i[1]-y_pole),(i[0]))
				r_obst = math.sqrt((i[0])**2+(i[1]-y_pole)**2)
				dist_end = math.sqrt(r_path**2+r_obst**2-2*r_path*r_obst*math.cos(theta2-theta_obst))
				if r_obst > r_path:
					r = r_obst-r_path
				else:
					r = max(abs(r_obst-r_path),dist_end)
		...
		#print('here')
		for i in obst.copy():
			#print(v * config.predict_time + config.robot_radius)
			if 0 <= i[0] <= v * config.predict_time:
				r = abs(i[1])
			#elif v * config.predict_time < i[0] < v * config.predict_time + config.robot_radius:
			#	r = math.sqrt((i[0]-(v * config.predict_time))**2+(i[1])**2)
			#	print(r)

			else:
				r = r = math.sqrt((i[0]-(v * config.predict_time))**2+(i[1])**2)
			if r < min_r:
				min_r = r
	#print(min_r)
...
def calc_obstacle_cost2(traj, obst, config,v,w):
	minr = 1000
	#print(obst.copy())
	if len(obst.copy()) == 0:
		#print("no detacted")
		...
		for i in obst.copy():
			if w < 0:
				x_path = 0
				y_path = -v/w
				theta_path = math.atan2(i[1]-y_path,i[0]-x_path)
				d_path = math.sqrt((i[0]-x_path)**2+(i[1]-y_path)**2)
				#print(-0.5*math.pi-w*config.predict_time)
				#print(d_path)
				#if -0.5*math.pi < theta_path and theta_path < -0.5*math.pi-w*config.predict_time:
				if -0.5*math.pi < theta_path and theta_path < -0.5*math.pi-w*config.predict_time:
					r = abs(d_path-abs(v/w))
					if r <= minr:
						minr = r
			elif w > 0:
				x_path = 0
				...
				#print(theta_path)
				#if 0.5*math.pi < theta_path and theta_path < 0.5*math.pi-w*config.predict_time:
				#if 0.5*math.pi-w*config.predict_time < theta_path and theta_path < 0.5*math.pi:
				if 0.5*math.pi-w*config.predict_time-math.asin(w*1.2/v) < theta_path < 0.5*math.pi:
					r = abs(d_path-abs(v/w))
					#print(r)
					if r <= minr:
						minr = r
			else:
				x_end = v * math.cos(w) * config.predict_time
				y_end = v * math.sin(w) * config.predict_time
				#print(x_end,y_end)
				r = math.sqrt((i[0]-x_end)**2+(i[1]-y_end)**2)
				if r <= minr:
					minr = r
			#print(d_path)
...
def calc_to_goal_cost(v,w,y_pole,r_path,theta1,theta2,config):
	ABT = np.array([[math.cos(config.th),-math.sin(config.th),0,config.x],
					[math.sin(config.th),math.cos(config.th),0,config.y],
					[0,0,1,0],
					[0,0,0,1]])
	...
	if w < 0:
		...
		x_end = v * config.predict_time
		y_end = 0
	x_end,y_end = np.dot(ABT,[x_end,y_end,0,1])[0],np.dot(ABT,[x_end,y_end,0,1])[1]

	total = math.sqrt((config.startX-config.goalX)**2 + (config.startY-config.goalY)**2)
	remain = math.sqrt((config.goalX-x_end)**2 + (config.goalY-y_end)**2)
	cost = 1 -(remain / total)
	return cost
...
def calc_odom_angle_cost0(traj, config):
	# If-Statements to determine negative vs positive goal/trajectory position
	# traj[-1,0] is the last predicted X coord position on the trajectory
	angle_end1 = math.atan2(config.goalY-traj[-1,1],config.goalX-traj[-1,0])#-traj[-1,2]
	angle_end2 = traj[-1,2]
	cost = 1-abs((angle_end1-angle_end2)/math.pi)
	return cost

def calc_odom_angle_cost(v,w,y_pole,r_path,theta1,theta2,config):
	ABT = np.array([[math.cos(config.th),-math.sin(config.th),0,config.x],
					[math.sin(config.th),math.cos(config.th),0,config.y],
					[0,0,1,0],
					[0,0,0,1]])
	...
	angle_end1 = math.atan2(config.goalY-y_end,config.goalX-x_end)#-traj[-1,2]
	angle_end2 = config.th+w * config.predict_time
	cost = 1-abs((angle_end1-angle_end2)/math.pi)
...
def main():
	...
	print("pole",t,config.predict_time,config.laser_reso)
	profiler.stop()
	...
	f = open("pole"+"_"+str(t)+"_"+str(config.predict_time)+"_"+str(config.laser_reso)+"_"+date+'.txt','w')
