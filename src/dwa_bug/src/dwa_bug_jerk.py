...
class Config():
	...
	def __init__(self):
		...
		self.max_jerk = 0.5  # [m/sss]
		self.j_reso = self.max_jerk/5
		self.v_reso = 0.1  # [m/s] 1d method
		#self.v_reso = 0.1    # [m/s] 2d method

		self.yawrate_reso = self.max_dyawrate/20.0#0.2 # [rad/s]
...
def motion(x, u, dt):
	...
	x = np.dot(DT,x)
	return x
def velocity_generator(j,config):
	time = 0.1
	a_predict_list = []
	v_predict_list = []

	a_predict_list.append(config.a_all_list[-1])
	v_predict_list.append(config.v_all_list[-1])
	while time <= config.predict_time:
		if a_predict_list[-1]+j*config.dt >= 0:
			a_predict_list.append(min(config.max_accel,a_predict_list[-1]+j*config.dt))
		else:
			a_predict_list.append(max(-config.max_accel,a_predict_list[-1]+j*config.dt))
		if v_predict_list[-1]+a_predict_list[-1]*config.dt >= 0:
			v_predict_list.append(min(config.max_speed,v_predict_list[-1]+a_predict_list[-1]*config.dt))
		else:
			v_predict_list.append(max(0,v_predict_list[-1]+a_predict_list[-1]*config.dt))
		time = time + config.dt
		#print(time)
	...
	return v_predict_list,a_predict_list[0]
...
def calc_final_input(x, u, dw, config, obst,t):
		...
		for w in np.arange(dw[2], dw[3]+0.5*config.yawrate_reso, config.yawrate_reso):
			...
			if v_predict_list[1] < 1.0:
				...
				if to_goal_dist >= deaccel_dist:
					...
					final_cost = (0.0*odom_angle_cost+1.0*to_goal_cost+2.0*ob_cost +0.1*speed_cost+0.01*jerk_cost)
				else:
					speed_cost = 1-abs(v_predict_list[-1]/config.max_speed)
					final_cost = (0.0*odom_angle_cost+1.0*to_goal_cost+2.0*ob_cost +10.0*speed_cost+0.01*jerk_cost)
					state = 1
			else:
				jerk_cost = -(j/config.max_jerk)
				if to_goal_dist >= deaccel_dist:
...
def dwa_control(x, u, config,obst,t):
	...
	u,state = calc_final_input(x, u, dw, config, obst,t)
	return u,state
...
def jerkxydgraph(config,odomx,odomy,jl):
	...
	fig = plt.figure(figsize=(7.5,5.0),tight_layout = True)
	...
	mappable = ax.scatter(x=odomx,y=odomy,c=jl2,label='Velocity',s=1.5,cmap=newcm,vmin=0,vmax=1)
	ax.set_aspect("equal")
	ax.set_xlabel('x [m]')
	ax.set_ylabel('y [m]')
	ax.set(facecolor=[0.5,0.5,0.5])
	ax.set_xlim(-32*0.7,32*0.7)
	ax.set_ylim(-18,18)
	ax.imshow(img,extent=[-32,32,-18,18])
	#ax.imshow(img,extent=[-32*0.7,32*0.7,-18*0.7,18*0.7])
...
def twodgraph(config,vl,al,jl):
	...
	ax_velocity.set_ylabel('Velocity [m/s]',fontsize=20)
	...
	ax_acceleration.set_ylim(-config.max_accel*2.0,config.max_accel*2.0)
	ax_jerk.set_ylim(-config.max_jerk*2,config.max_jerk*2)
	#fig.savefig("lin35-2")
...
def main():
	...
	while not rospy.is_shutdown():
		...
		if (atGoal(config,x) == False) and (sl[-1]!=-1):#local minimum test use
			...
			u,state = dwa_control(x, u, config, obs.obst,t)
			x[0] = config.x
			x[1] = config.y
			x[2] = config.th
			x[3] = u[0]
		...
		jl.append(u[2])
		wl.append(speed.angular.z)
		awl.append((wl[-1]-wl[-2])/config.dt)
		jwl.append((awl[-1]-awl[-2])/config.dt)
	...
	print(config.v_all_list.index(2.0))
	#config.v_all_list = config.v_all_list[config.v_all_list.index(2.0):]
	print(config.v_all_list.index(2.0),td[0])
	print(sum(config.v_all_list[config.v_all_list.index(2.0):td[0]])/len(config.v_all_list[config.v_all_list.index(2.0):td[0]]))
