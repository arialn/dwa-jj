...
class Config():
	...
	def __init__(self):
		...
		self.av_reso = self.max_accel/5    # [m/s] 2d method
		self.aw_reso = self.max_dyawrate/5
...
def motion(x, u, dt):
	...
	x = np.dot(DT,x)
	return x
def linear_velocity_generator(av,config):
	time = 0.1
	#a_predict_list = []
	v_predict_list = []
...
def angular_velocity_generator(aw,config):
	time = 0.1
	#a_predict_list = []
	w_predict_list = []
...
def calc_final_input(x, u, dw, config, obst):
	...
	#for v in np.arange(dw[0], dw[1]+0.5*config.v_reso,config.v_reso):
		for aw in np.arange(-config.max_dyawrate, config.max_dyawrate+0.5*config.aw_reso,config.aw_reso):
			v_predict_list = linear_velocity_generator(av,config)
			w_predict_list = angular_velocity_generator(aw,config)
			...
			#speed_cost = traj[-1, 3]/config.max_speed
			#final_cost = (1.0*odom_angle_cost+1.0*to_goal_cost+2.0*ob_cost +2.0*speed_cost+0.001*jerk_cost)
			"""
			...
			waccel_cost = 1-(abs(aw/config.max_dyawrate))
			if v_predict_list[1] < config.max_speed/2:
				accel_cost = 1-(abs(av/config.max_accel))
				#jerk_cost = (j/config.max_jerk)
				if to_goal_dist >= deaccel_dist:
					speed_cost = traj[-1, 3]/config.max_speed
					final_cost = (0.0*odom_angle_cost+1.0*to_goal_cost+2.0*ob_cost +1.0*speed_cost+0.01*accel_cost+0.01*waccel_cost)
			...
			if final_cost >= max_cost:
				max_cost = final_cost
				max_u = [v_predict_list[1], w_predict_list[1]]
...
def main():
	...
	while not rospy.is_shutdown():
		...
		config.av_all_list.append((config.v_all_list[-1]-config.v_all_list[-2])/config.dt)
		config.w_all_list.append(speed.angular.z)#velocity

		#config.a_all_list.append((config.v_all_list[-1]-config.v_all_list[-2])/config.dt)#acceleration
		config.aw_all_list.append((config.w_all_list[-1]-config.w_all_list[-2])/config.dt)
		#print(u[2])
		#jl.append(u[2])
		#jl.append((config.v_all_list[-1]-config.v_all_list[-2])/config.dt)
		vjl.append((config.av_all_list[-1]-config.av_all_list[-2])/config.dt)#jerk
