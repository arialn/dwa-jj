...
def calc_final_input(x, u, dw, config, ob):
	...
	for v in np.arange(dw[0], dw[1]+config.v_reso,config.v_reso):
		for w in np.arange(dw[2], dw[3], config.yawrate_reso):
			...
			current_to_goal_cost = current_calc_to_goal_cost(config)

			speed_cost = traj[-1, 3]/config.max_speed

			ob_cost = calc_obstacle_cost(traj, ob, config)
			currrent_ob_cost = current_calc_obstacle_cost(obst, config)

			odom_angle_cost = calc_odom_angle_cost(traj,config,x)
			traj_angle_cost = calc_traj_angle_cost(traj,config,x)

			final_cost2 = 0.1 * odom_angle_cost + 4.0*ob_cost +1.0*to_goal_cost + 0.1* traj_angle_cost + 2.5*speed_cost
			if  final_cost2 >= max_cost:
				max_cost = final_cost2
				max_u = [v, w]
				return_cost = odom_angle_cost
...
def calc_odom_angle_cost(traj, config,x):
	# If-Statements to determine negative vs positive goal/trajectory position
	# traj[-1,0] is the last predicted X coord position on the trajectory
...
def calc_traj_angle_cost(traj, config,x):
	# If-Statements to determine negative vs positive goal/trajectory position
	# traj[-1,0] is the last predicted X coord position on the trajectory
	angle_start = x[2]
	angle_end = traj[-1,2]
...
def atGoal(config, x):
	# check at goal
	if math.sqrt((x[0] - config.goalX)**2 + (x[1] - config.goalY)**2) <= 1.0:
		return True
	return False
...
def jerkxydgraph(odomx,odomy,jl):
	...
	ax.set_xlabel('x[m]')
	ax.set_ylabel('y[m]')
	ax.set(facecolor=[0.9,0.9,0.9])
	#ax.set_ylim(0,None)
	pp = fig.colorbar(mappable,cax=cax,label='Jerk['+'m/$ \mathit{s}^{3}$'+']')
	#pp.set_clim(0,np.mean(jl)+1*np.var(jl))
	...
	for i in range(z_mesh.shape[0]) :
		for j in range(z_mesh.shape[1]) :
			if z_mesh[i,j] >= 1.0:
				z_mesh[i,j] = 1.0
	return z_mesh
def map_to_potential(source_path):
	...
	#gray = cv2.resize(gray,(64,36),0,0,cv2.INTER_AREA)
	...
	#print(type(x_mesh),type(y_mesh),type(z_mesh))
	#print(x_mesh)
	#print(y_mesh)
	...
	#z_mesh = np.zeros((64,36))
	#z_mesh[63,18] = 10.0
	fig = plt.figure()
	sub = fig.add_subplot(111,projection='3d')
	surf = sub.plot_surface(x_mesh,y_mesh,z_mesh,cmap = plt.cm.Purples)
...
def draw_traj():
	...
	#for v in np.arange(vd[0],vd[1]+v_reso,v_reso):
	for v in [vd[1]]:
		#print(v)
		for w in np.arange(wd[0],wd[1]+0.1,w_reso):
			...
			while time < predict_time:
				x.append(x[i] + v * math.cos(theta[i]) * dt)
				y.append(y[i] + v * math.sin(theta[i]) * dt)
...
if __name__ == '__main__':
	...
	x = np.linspace(-32,32,64)
	y = np.linspace(-16,16,32)
	ox = np.linspace(-5,2,7)
	oy = np.linspace(-7,7,14)
	...
	#remain = math.sqrt((config.goalX-traj[-1,0])**2 + (config.goalY-traj[-1,1])**2)
	x_mesh,y_mesh = np.meshgrid(x,y,indexing='ij')
	#ox_mesh,oy_mesh = np.meshgrid(ox,oy,indexing='ij')
	#print(x_mesh)
	z_mesh = np.ones((60,32))

	ob_mesh = show_to_goal_cost()
	total_mesh = ob_mesh + z_mesh
	fig = plt.figure()
	sub = fig.add_subplot(111,projection='3d')
	surf = sub.plot_surface(x_mesh,y_mesh,z_mesh,cmap = plt.cm.Purples)
	cb = fig.colorbar(surf)
	#C = sub.contour(x_mesh,y_mesh,total_mesh)
	#sub.clabel(C)
	plt.show()
	"""
	#source = "/home/z_lin/tbug/src/tangentbug/world/lab.png"
	#map_to_potential(source)
	#draw_traj()
	#print(x,y)
