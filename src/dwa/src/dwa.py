...
class Config():
    ...
    def goalCB(self,msg):
        self.goalX = 2#msg.linear.x
        self.goalY = 0#msg.linear.y
...
def main():
    ...
    while not rospy.is_shutdown():
        if (atGoal(config,x) == False):
            ...
            speed.linear.x = 0.0
            speed.angular.z = 0.0
        with open("/home/z_lin/Desktop/ROS-Dynamic-Window-Approach-master/old_dwa_line.csv","a+")as f:
        #with open("/home/z_lin/Desktop/ROS-Dynamic-Window-Approach-master/dwa_sline.txt","a+")as f:
           f.write(str(speed.linear.x)+","+str(speed.angular.z)+"\n")
           f.close()
        pub.publish(speed)
        config.r.sleep()
